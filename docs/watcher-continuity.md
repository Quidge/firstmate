...
`bin/fm-watch-arm.sh` never returns a clean empty success: every exit carries an explicit status line.
...
A matching PID and identity lets an attached arm report the delivered reason and exit zero even after the durable wake was handled and acknowledged, while an unrelated queue producer or a recycled PID cannot satisfy the match.
When a cycle ends with no matching delivery record and no verified healthy successor, the outcome depends on ownership.
An arm that owned the singleton - its own started child was confirmed healthy - emits `watcher: FAILED - cycle ended without an actionable reason` and exits nonzero.
An arm that only ever attached to a live peer, never owning the singleton, instead stands down cleanly with `watcher: stood down (benign concurrent re-arm; ...)` and exits 0: a redundant duplicate from a concurrent re-arm is not the supervision owner, so it leaves continuity to that peer's owner arm and to the independent drain-time and turn-end liveness guards rather than raising a spurious supervision-off alarm (Quidge/firstmate#2).
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, the benign concurrent-restart stand-down of an attach-only arm versus the loud failure of an owning arm, bounded and successor-linked lifecycle rows, and a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination.
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
`tests/fm-watch-triage.test.sh` proves a deferred enqueue in one poll cannot suppress a later actionable fallback after benign-signal marker publication fails.
...
[`verification/supervision.md`](verification/supervision.md#watcher-continuity) records the five-harness continuity matrix, [Cursor's primary-park evidence](verification/supervision.md#cursor-primary-park-2026-08-13), the 2026-07-24 Stop-owned Claude auto-arm results, and exact opt-in commands.
