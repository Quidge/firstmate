...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it forks the watcher as a tracked child, verifies it is genuinely alive with a fresh liveness beacon, and prints an honest `started`, `attached`, nonzero `FAILED`, or benign `stood down` status.
On `attached` it stays live across identity-matched successors; when a cycle ends without a healthy successor it first resolves the close against the watcher's terminal-delivery ledger, and only then does an unmatched close split by ownership: an arm that owned the singleton becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result, while an arm that only attached to a live peer stands down cleanly (`watcher: stood down`, exit 0) and leaves continuity to that peer's owner.
[`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the arm layer's successor, terminal-delivery, ownership stand-down, and typed clean-close failure contract.
...
Memory writes use inspect-then-update rather than blind append; the internal [`stow` skill](../.agents/skills/stow/SKILL.md) owns tier markers, decay, cold archival, and captain-gated offload.
