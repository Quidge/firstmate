...
- **Explicit delivery modes** - each ship task delivers via `no-mistakes`, `direct-PR`, or `local-only`, chosen at intake with the project's registered posture as the default, plus an optional `+yolo` autonomy flag.
