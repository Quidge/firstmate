...
- **Explicit project delivery modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, chosen at intake with the project's registered posture as the default, plus an optional `+yolo` merge-autonomy flag.
