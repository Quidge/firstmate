...
# FORK DIVERGENCE (Option A, decision key resync-fixb-verdict): upstream #1545
# asserted a loud "watcher: FAILED" for this terminal state. This fork instead
# stands an attach-only (non-owning) arm down cleanly when its observed peer ends
# having delivered no wake of its own and left no successor: the peer's owner arm
# and the drain-time and turn-end liveness guards hold continuity, so a redundant
# duplicate must not fuel the false-FAILED re-arm churn the fork's Fix B was
# written to suppress (see docs/watcher-continuity.md). Only this benign
# concurrent re-arm stands down; an OWNING arm on an unexplained close still fails
# loudly. Re-reconcile this deliberate divergence knowingly on future upstream
# syncs.
test_attached_arm_stands_down_on_a_wake_it_did_not_deliver() {
  ...
  # A process-event producer advances the same home-wide queue while the observed
  # watcher remains uninvolved, so no watcher-bound delivery record matches this
  # cycle. An attach-only arm resolves that unmatched close as a benign stand-down.
  append_wake "$state" check process-event "check: process-event result captured: fixture"
  kill "$SEED_PID" 2>/dev/null || true
  wait "$SEED_PID" 2>/dev/null || true
  wait_for_exit "$ARM_PID" 120
  status=$?
  grep -qF 'watcher: stood down (benign concurrent re-arm' "$armout" \
    || fail "an attach-only arm on a cycle it did not deliver must stand down benignly: $(cat "$armout")"
  ! grep -qF 'watcher: FAILED' "$armout" \
    || fail "a benign concurrent re-arm must not report a loud failure: $(cat "$armout")"
  expect_code 0 "$status" "an attach-only arm that observed no delivery and no successor stands down cleanly"
  pass "watch-arm: an attach-only cycle that delivered no wake of its own stands down benignly, not FAILED"
}

# Pins the composed contract's ORDERING: close_unobserved_cycle consults the
# durable delivery ledger BEFORE the ownership-based stand-down. An attach-only
# arm whose cycle DID durably deliver a wake must report that signal - never stand
# down - even though it never owned the singleton. If the ownership split ran
# first, this attach-only arm would stand down and silently swallow a real
# delivered wake. Ledger-first, then ownership-split, is the whole point of the
# integration, and this case fixes that order in place.
test_ledger_lookup_precedes_the_benign_standdown() {
  local dir state fakebin out armout status
  dir=$(make_case ledger-before-standdown)
  state="$dir/state"
  fakebin="$dir/fakebin"
  out="$dir/watch.out"
  armout="$dir/arm.out"
  ...
  # The observed watcher delivers a real captain-relevant wake, publishes it to
  # the identity-bound delivery ledger before releasing its lock, then exits with
  # no successor - exactly the attach-only, no-successor shape the benign
  # stand-down also handles, so the two paths are distinguished only by order.
  printf 'done: fixture finished\n' > "$state/demo.status"
  wait_for_exit "$SEED_PID" 120
  grep -q '^signal:' "$out" || fail "seed watcher did not surface the signal wake: $(cat "$out")"
  ...
  grep -q 'demo.status' "$state/.wake-queue" \
    || fail "the wake was not durably recorded, so this case proves nothing"
  grep -q '^signal:' "$armout" \
    || fail "ledger-first must report the durably delivered wake: $(cat "$armout")"
  ! grep -qF 'watcher: stood down' "$armout" \
    || fail "the delivery ledger must be consulted before the stand-down; a delivered wake was swallowed: $(cat "$armout")"
  ! grep -qF 'watcher: FAILED' "$armout" \
    || fail "a delivered wake must not report a loud failure: $(cat "$armout")"
  expect_code 0 "$status" "an attach-only arm whose cycle delivered a wake reports it via the ledger and exits cleanly"
  pass "watch-arm: the delivery ledger is consulted before the ownership stand-down, so a delivered wake is never swallowed"
}
