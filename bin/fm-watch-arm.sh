...
# that ends with no reason line and no healthy successor is first resolved against
# the watcher's identity-bound delivery record: a matching record reports that
# wake and exits 0. If nothing was delivered, the outcome then depends on
# ownership: an arm that owned the singleton fails loudly (typed nonzero, never a
# clean empty completion), while a benign concurrent re-arm that only attached to
# a live peer stands down cleanly and leaves continuity to that peer's owner
# (Quidge/firstmate#2). On FAILED it exits non-zero so the failure is loud. A live
...
# Look up the wake this cycle durably delivered against the bounded
# terminal-delivery ledger the watcher publishes before releasing its lock.
# Prints the delivered reason and returns 0 on a matching record; returns 1
# WITHOUT printing anything when no record matches or the ledger lock cannot be
# taken, so the caller alone decides whether an unmatched close is a loud failure
# or a benign stand-down.
lookup_delivered_reason() {
...
# Close a cycle whose reason line this arm could not read. First honor the
# durable delivery ledger: a matching record reports that wake and exits cleanly.
# An unmatched close then splits by ownership - an arm that owned the singleton
# fails loudly, while a benign concurrent re-arm that only ever attached to a live
# peer stands down cleanly (Quidge/firstmate#2). Returns 0 when the arm may exit
# cleanly (a delivered wake or a benign stand-down) and 1 for a loud failure.
close_unobserved_cycle() {
  if lookup_delivered_reason; then
    return 0
  fi
